	...
	public boolean foldLineAble(int x1, int y1, int x2, int y2) {
		...
		for (int x = x1 - 1; x >= 0; x--) // 向左历遍
		{
			...
			if (linkSegment(x1, y1, x, y1)) {
				// 如果剩下两条也可以连接
				if (linkSegment(x, y1, x, y2) && linkSegment(x, y2, x2, y2)) {
					// 计算最小路程
					minDistance = Math.abs(x1 - x) + Math.abs(y2 - y1) + Math.abs(x2 - x);

					// 保存折点
					points = new int[] { x1, y1, x, y1, x, y2, x2, y2 };
					// 找到折线,不再往这个方向搜索
					break;
				}
			} else { // 遇到障碍,不再往这个方向搜索
		...
		for (int x = x1 + 1; x < blocks[0].length; x++) // 向右历遍
		{
			...
			if (linkSegment(x1, y1, x, y1)) {
				...
				if (linkSegment(x, y1, x, y2) && linkSegment(x, y2, x2, y2)) {
					// 计算最小路程
					int temp = Math.abs(x1 - x) + Math.abs(y2 - y1) + Math.abs(x2 - x);
		...
		for (int y = y1 + 1; y < blocks.length; y++) // 向下历遍
		{
			...
			if (linkSegment(x1, y1, x1, y)) {
				// 如果剩下两条也可以连接
				if (linkSegment(x1, y, x2, y) && linkSegment(x2, y, x2, y2)) {
					// 计算最小路程
					int temp = Math.abs(y - y1) + Math.abs(x2 - x1) + Math.abs(y - y2);

					if (temp < minDistance || minDistance == 0) {
						minDistance = temp;
						// 保存折点
						points = new int[] { x1, y1, x1, y, x2, y, x2, y2 };
					}
					break;
				}
			} else {
