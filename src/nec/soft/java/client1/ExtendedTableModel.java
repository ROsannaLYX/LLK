...
class ExtendedTableModel extends DefaultTableModel {
	...
	// 重新提供一个构造函数，该构造函数的实现委托给DefaultTableModel父类
	...
	public void reSetTable(Object[] columnNames, List<UserInfo> userInfo) {
		...
		for (int i = 0; i < userInfo.size(); i++) {
			cells[i] = new Object[] { userInfo.get(i).getIconIndex(), userInfo.get(i).getName(),
					userInfo.get(i).getSex(), userInfo.get(i).getScore() };
	...
	public void addRow(UserInfo user) {
		Object[] rowData = new Object[] { user.getIconIndex(), user.getName(), user.getSex(),
				user.getScore() };
		addRow(rowData);
