...
public class IconTable extends JTable
{
	...
	public IconTable(ExtendedTableModel model
		, int iconCol , int tableStyle )
	{
		super(model);
	...
	public void init()
	{
		...
		if(tableStyle == GENERAL && iconCol != -1)
		{
	...
	class GenderTableCellRenderer extends JPanel implements TableCellRenderer
	{
		...
		private void drawImage(Graphics g , Image image)
		{
			...
			{
				g.drawImage(image, (getWidth() - ICON_WIDTH ) / 6
					, (getHeight() - ICON_HEIGHT) / 6 , 40 , 40 , null);
			}
			else
			{
				g.drawImage(image, (getWidth() - ICON_WIDTH ) / 6
					, (getHeight() - ICON_HEIGHT) / 6 ,null);
			}
